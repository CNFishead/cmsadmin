  ...
  const { mutate: updateNotification } = useApiHook({
    ...
    url:
      props.notification._id !== ''
        ? `/notification/${props.notification._id}`
        : `/notification/all`,
  }) as any;
  return (
    <>
      ...
      >
        <div className={styles.content}>
          <div className={styles.titleContainer}>
            ...
            <h1 className={`${styles.title}  ${props.small ? styles.small : ''}`}>
              {props.notification.message}
            </h1>
          </div>

          <p className={`${styles.description} ${props.small ? styles.small : ''}`}>
            {props.notification.description}
          </p>
        </div>
        <div className={styles.date}>
          {moment(props.notification?.createdAt).format('MM/DD/YYYY').toString()}
        </div>
      </Link>
    </>
  );
